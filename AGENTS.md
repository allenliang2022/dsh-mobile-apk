...
| 时间 | 版本 | 更新内容 | 更新者 |
|---|---|---|---|
| 2026-09-13 | native-proot（未发布草稿） | ARM64 APK-native PRoot/loader + 原子路径/启动器部署；干净 guest 环境与 sdcard 显式选择；四构建入口 ABI 绑定与 APK 产物校验；来源未核实的 x86 预编译不分发；随 APK 附对应源码/重建输入/许可；无用户 rootfs 自动迁移，完整构建/双机验收与私有协调仓镜像仍待完成（详 NATIVE-PROOT.md） | AI 开发助手 |
