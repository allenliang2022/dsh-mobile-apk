...
| 时间 | 版本 | 更新内容 | 更新者 |
|---|---|---|---|
| 2026-09-13 | native-proot（验收补充，未发布） | ARM64 APK-native PRoot/loader、原子状态/启动器与精确 ABI/source 产物门禁；新增隔离 Android instrumentation、真实 APK 升级工作区保持检查与事务回滚/恢复测试；协调布局 CLI/镜像自校验与递归负控、13 文件配套同步清单；API26/28 helper 与 API35 升级仪器验证已过（具体 revision 见 NATIVE-PROOT.md）；固定输入的 NDK 独立重建已验证编译及结构，字节不等于随包二进制且不自动替换；标准 ARM64 macOS 首次探针已到 boot_completed，但通用解锁按键失败而未运行 APK 测试；本实验分支改用 AOSP、无 UI 输入、稳定 PM/AM 与实际墙钟预算（见 NATIVE-PROOT-HEADLESS-PROBE.md，不冒充来宾验收）；设备验收以 NATIVE-PROOT.md 的具体证据为准，不把 synthetic peer 当真实同步；rootfs 不自动迁移 | — |
