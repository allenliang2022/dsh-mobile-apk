...
- **升级保留边界（恢复保护开发分支）**：`usr` 可能含现场自装工具和guest，不能视作可丢弃工厂目录；成功更新后完整旧树保留到私有 `workspaces/upgrade-recovery`，不自动清理/恢复/执行；`RETAINING` 只前滚，旧 `debian` 入口无覆盖重建。安装前必须验证备份时效、原入口/挂载/认证/真实工作流；详见 `docs/UPGRADE-RECOVERY.md`，源测试通过不等于已部署保护。
- **无线调试修复分支（未发布）**：bridge 0.2.5；按类型发现本机端点、独立 NSD 监听器、强制新扫描、三态后台观测、异步操作票据与有界服务器恢复；当前规范和回归入口见 `docs/AGENTS/adb-chain.md`。不得把配对偏好、TCP 可达、编译成功或模拟器接口测试冒充系统开关/真实配对验收。
...
| 时间 | 版本 | 更新内容 | 更新者 |
|---|---|---|---|
| 2026-09-13 | native-proot（验收补充，未发布） | ARM64 APK-native PRoot/loader、原子状态/启动器与精确 ABI/source 产物门禁；新增隔离 Android instrumentation、真实 APK 升级工作区保持检查与事务回滚/恢复测试；协调布局 CLI/镜像自校验与递归负控、13 文件配套同步清单；API26/28 helper 与 API35 升级仪器验证已过（具体 revision 见 NATIVE-PROOT.md）；固定输入的 NDK 独立重建已验证编译及结构，字节不等于随包二进制且不自动替换；新增标准 ARM64 macOS 软件模拟器限时可行性探针（不冒充来宾验收）；设备验收以 NATIVE-PROOT.md 的具体证据为准，不把 synthetic peer 当真实同步；rootfs 不自动迁移 | — |
