...
- **定点 ADB 运行时 patch（开发中）**：`runtime-patches/adb-local-listener/` 提供 ELF 兼容入口、哈希/路径/类型校验、事务应用及定点撤销；不要求 APK 升级，不修改模型/配置/授权或现有密钥。只替换已核验的 `usr/bin/adb`，保留原件。原生源码修复与该兼容层分开验收；应用真实环境前必须获确认。测试载体 APK 仅在一次性 CI 模拟器使用，不是 DSH 更新包。
